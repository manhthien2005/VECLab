...
- **Zone 4: Privileged Database Boundary**
  ...
  - Server RPCs perform explicit actor ownership checks as applicable. Write paths use row locking (`SELECT ... FOR UPDATE`) where required for atomicity and concurrency serialization (e.g. `commit_attempt_event`, `create_attempt`, `complete_attempt`, `stop_attempt`, `branch_attempt`, `delete_attempt`), while read-only RPCs (`get_attempt`, `list_attempts`) are ownership-filtered and do not use row locks.
...
| SECURITY DEFINER abuse | DB execution privilege | RPC call via service role | Attacker passes arbitrary arguments | Explicit actor ID validation, locked row checks (on write RPCs), and immutability triggers | [supabase/migrations/20260909000600_commit_event_rpc.sql](supabase/migrations/20260909000600_commit_event_rpc.sql#L98-L135) | Compromised BFF invokes RPC | CONTROLLED |
...
- **T06 SECURITY DEFINER Abuse**:
  - *Control*: RPCs parameterize queries, validate caller-supplied actor user ID against row ownership as appropriate, and enforce immutability triggers; write RPCs lock rows to serialize validation and updates ([supabase/migrations/20260909000600_commit_event_rpc.sql#L98-L135](supabase/migrations/20260909000600_commit_event_rpc.sql#L98-L135)).
  - *Status*: `CONTROLLED`.
...
- **Locking semantics across RPCs**:
  ...
  - `create_attempt`: executes `SELECT ... FOR UPDATE` on `p_attempt_id` ([supabase/migrations/20260909000600_commit_event_rpc.sql#L330](supabase/migrations/20260909000600_commit_event_rpc.sql#L330)) to handle idempotent retries and prevent race conditions on attempt ID collision.
  - `branch_attempt`: locks both the new attempt ID and the parent attempt row via `SELECT ... FOR UPDATE` ([supabase/migrations/20260909000600_commit_event_rpc.sql#L629,L648](supabase/migrations/20260909000600_commit_event_rpc.sql#L629)) to verify parent state and prevent duplicate branching.
